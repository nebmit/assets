...
import { mkdir, open, rename, rm, mkdtemp, link, readFile } from 'node:fs/promises';
	...
	for (let attempt = 0; ; attempt++) {
		try {
			...
			if (!res.ok) {
				...
				const delay = retryDelay(res.headers.get('retry-after'), attempt);
				await res.body?.cancel();
				if (retry && attempt < 3) { await pause(delay); continue; }
				throw new HttpError(res.status, url, 'source request failed');
			}
			return await consume(res);
		} catch (error) {
			if (error instanceof SecAccessError || error instanceof HttpError || attempt >= 3) throw error;
...
export async function archiveEvidence(url: string, content: string, root = config().RAW_DATA_DIR): Promise<Evidence> {
	...
	try {
		...
		try { await link(path.join(temp, 'body'), target); }
		catch (error) {
			if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error;
			if (hash(await readFile(target)) !== digest) throw new Error('corrupt SEC evidence object');
		}
		...
		await request(url, async (res) => {
			...
			try {
				for await (const chunk of bodyChunks(res.body)) {
					...
					if (size > 8 * 1024 ** 3) throw new Error('SEC ZIP exceeds 8 GiB');
