<script lang="ts">
	 ...
	 */
	interface Props {
		cards: Pick<CardData, 'assetId' | 'isin' | 'ticker' | 'links' | 'name' | 'wkn' | 'sector'>[];
		query: string;
		onignore: (card: Pick<CardData, 'assetId' | 'name'>) => void;
	}
...
<section
	...
	aria-label="Assets matching the search"
>
	<header
	...
	{:else}
		<ul class="m-0 list-none p-0">
			{#each cards as card (card.assetId)}
				<li
				...
				>
					...
					>
						...
						<span class="font-mono tabular-nums">
							<Link href={card.links.quote ?? card.links.filings ?? '#'} external variant="quiet" size="xs">
								{card.ticker ?? card.isin ?? ''}{#if card.wkn !== null}&nbsp;· {card.wkn}{/if}
							</Link>
						</span>
					</div>
					{#if card.sector !== null}
