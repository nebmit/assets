...
import { indexMembership, insiderTransaction, instrument, issuer, shortPositionSnapshot } from '../src/lib/server/db/schema.js';
...
describe.skipIf(!url)('short seller snapshots through engine, feed and MCP', () => {
	...
	beforeAll(async () => {
		...
		for (const [i, isin] of isins.entries()) {
			...
			const [inst] = await handle.db.insert(instrument).values({ issuerId: iss.id, isin, firstSeen: RUN_DATE, lastSeen: RUN_DATE }).returning();
			...
			if (i < 2) await handle.db.insert(insiderTransaction).values({
				...
				side: 'buy', instrumentType: 'Aktie', amount: '1000000', transactionDate: RUN_DATE,
	...
	it('selects snapshots by Berlin day, including DST, and excludes future observations', async () => {
		...
		expect((await loadShortSellerAnalysis(handle.db, '2026-12-01')).get(issuerIds[0])?.totalDisclosedPct).toBe(0.7);
	});
	...
	it('keeps card, watchlist and MCP evidence identical after newer ingestion', async () => {
		...
		expect(feed.shortSellersByIsin[isins[2]].status).toBe('none_disclosed');
		...
		expect(card.shortSellers).toEqual(feed.shortSellersByIsin[isins[0]]);
		...
		expect((await issuerDetail(handle.db, card.isin, RUN_DATE))?.shortSellers).toEqual(card.shortSellers);
		expect(feed.cardsByView.surfaced[0].isin).toBe(isins[1]);
		// Explicit regeneration replaces evidence atomically.
		await runSignals(handle.db, RUN_DATE);
		expect((await loadFeed(handle.db))?.shortSellersByIsin[isins[0]].status).toBe('none_disclosed');
	});
	...
	it('returns watchlist analysis even when the discovery feed is empty', async () => {
		...
		expect(Object.keys(feed.shortSellersByIsin)).toHaveLength(3);
	});

	it('handles no coverage and pre-feature signal rows without inventing absence', async () => {
		await runSignals(handle.db, RUN_DATE);
		let feed = (await loadFeed(handle.db))!;
		expect(feed.shortSellersByIsin[isins[0]].status).toBe('unknown');
		...
		expect(feed.shortSellersByIsin).toEqual({});
